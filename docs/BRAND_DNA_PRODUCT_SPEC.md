---
name: Brand voice channels
overview: Planning only. Preferred capture is an 8–10 minute audio interview (local open STT), confirmed on screen; paste/type is backup. First future slice is still Stage 1–2. No code until you ask to build.
todos:
  - id: usp-loop
    content: Productize the expertise loop — capture what only the user knows, then generate
    status: pending
  - id: brand-as-strategy
    content: Treat each user brand as a 1-page content strategy (audience, help, POV, refuse)
    status: pending
  - id: channel-fit
    content: Place expertise on the channel where it is useful, not clone 27 mediocre posts
    status: pending
  - id: voice-better
    content: Keep their words and rhythm; raise hook, clarity, and CTA (like them, better)
    status: pending
  - id: compounding
    content: Learn from edits, audience language, and later connected posts so switching cost is the user themselves
    status: pending
isProject: false
---
...
```mermaid
flowchart TD
  brand[Brand_promise] --> expertise[Lived_expertise]
  expertise --> voice[Natural_voice]
  voice --> taste[Edits_and_refusals]
  taste --> posts[Channel_posts]
  posts --> audience[Audience_language]
  audience --> expertise
```
...
```mermaid
flowchart LR
  s1[Stage1_BrandPromise]
  s2[Stage2_Interview]
  s3[Stage3_VoiceStudy]
  s4[Stage4_OnePostOneRoom]
  s5[Stage5_EditLoop]
  s6[Stage6_Proof]
  s1 --> s2 --> s3 --> s4 --> s5 --> s6
```
...
flowchart LR
  newBrand[New_brand]
  talk[Talk_8min]
  confirm[Confirm_promise_and_words]
  paste[Or_type_and_paste]
  draft[Draft_later]
  newBrand --> talk --> confirm --> draft
  newBrand --> paste --> confirm
```
